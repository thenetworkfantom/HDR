...
HDR is a behavioral-based IPS that uses machine learning to detect malicious behaviors in the network traffic. It is a modular software that can be extended. When HDR is run, it spawns several child processes to manage the I/O, to profile attackers and to run the detection modules.

Here we describe what detection modules are run on the traffic to detect malicious behaviour.


Modules are Python-based files that allow any developer to extend the functionality of HDR. They process and analyze data, perform additional detections and store data in Redis for other modules to consume. Currently, HDR has the following modules:
...
<table>
  ...
  <tr>
    ...
    <td>Exports HDR alerts to Slack servers and STIX servers.</td>
    <td>✅</td>
  </tr>
  <tr>
    <td>IP Info</td>
  ...
  <tr>
    <td>Flow Alerts</td>
    <td>Finds malicious behaviours by analyzing only one flow. Now detects: self-signed certificates, TLS certificates which validation failed, vertical port scans detected by Zeek (contrary to detected by HDR), horizontal port scans detected by Zeek (contrary to detected by HDR), password guessing in SSH as detected by Zeek, long connection, successful ssh</td>
    <td>✅</td>
  </tr>
  <tr>
    <td>Leak Detector</td>
...
Example:
```commandline
# HDR computed value of the flow
...
Refer to the [Flowalerts section of the docs](https://stratospherelinuxips.readthedocs.io/en/develop/flowalerts.html) for detailed explanation of what HDR detects and how it detects.

## Disabled alerts

All HDR detections are turned on by default, You can configure which alerts you want to enable/disable in ```config/slips.conf```

HDR support disabling unwanted alerts, simply add the detection you want to disable in
...
HDR has a complex system to deal with Threat Intelligence feeds.

HDR supports different kinds of IoCs from TI feeds (IPs, IP ranges, domains, JA3 hashes, SSL hashes)

File hashes and URLs aren't supported in TI feeds.

Besides the searching 40+ TI files for every IP/domain HDR encounters, It also uses the following websites for threat intelligence:
...
HDR gets every IP it can find in the network (DNS answers, HTTP destination IPs, SSH destination IPs, etc.)
...
### Matching of Domains
HDR gets every domain that can find in the network and tries to see if it is in any blacklist.
...
HDR has a local file for adding IoCs of your own,
...
Refer to the [architecture section of the docs](https://stratospherelinuxips.readthedocs.io/en/develop/architecture.html) for detailed explanation of HDR threat levels.
...
HDR has a local file for adding JA3 hashes of your own,
...
By default, the files are stored in the HDR directory ```modules/ThreatIntelligence1/remote_data_files/```
...
HDR every 24 hours, which requires no user interaction.
...
HDR is shipped with an offline database (GeoLite2) in ```databases/GeoLite2-ASN.mmdb```
...
HDR is shipped with an offline database (GeoLite2) in ```databases/GeoLite2-Country.mmdb```
to search for Geolocation.

### Mac Vendors

HDR is shipped with an offline database ```databases/macaddress-db.json``` for
MAC address vendor mapping.

HDR updates this database by default every 2 weeks using the following online db
...
HDR gets the MAC address of each IP from dhcp.log and arp.log and then searches the offline
database using the OUI.

If the vendor isn't found in the offline MAC database,
HDR tries to get the MAc using the online database https://www.macvendorlookup.com

The offline database is updated manually and shipped with slips, you can find it in
the ```databases/``` dir.

HDR makes sure it doesn't perform duplicate searches of the same MAC Address either online, or offline.
...
By default, zeek doesn't generate and log ARP flows, but HDR is shipped with it's
...
HDR considers an IP performing an ARP scan if it sends 5
or more non-gratuitous ARP to different destination addresses in 30 seconds or less.

### ARP to a destination IP outside of local network

HDR alerts when an ARP flow is being sent to an IP outside of local network as it's a weird behaviour
...
HDR detects when a MAC with IP A, is trying to tell others that now that MAC
...
HDR imports 100 alerts from warden servers each day, and automatically stores the IoCs in our database
...
These are the categories HDR imports:
...
HDR has a list of suspicious user agents, whenever one of them is found in the traffic, slips generates
and evidence.
...
HDR uses and offline MAC address database to detect the type of device based on the MAC OUI.

First, HDR store the MAC address and vendor of every IP it sees (if available)
...
HDR stores the MAC address and vendor of every IP it sees
...
HDR detects downloads of files from pastebin through HTTP with size >= 700 bytes.
...
If you want to contribute, improve existing HDR detection modules or implement your own detection modules, see section :doc:`Contributing <contributing>`.
...
HDR considers an IP performing a vertical port scan if it contacts 5 or more different destination ports to the same destination IP in at least one time window (usually 1hs). The flows can be TCP or UDP, and both Established or Not Established. On each arriving flow this check is performed.

After detecting a vertical port scan for the first time, if HDR detects new flows to 5 destination ports, then it triggers a waiting process to find out how many packets to new ports will arrive. For this it waits 10 seconds to see if more flows arrive, since in most port scans the attcker will scan more ports. This avoids generating one alert 'port scan' per flow in a long scan. Therfore HDR will wait until the scan finishes to alert on it. However, the first portscan is detected as soon as it happens so the analysts knows.

If one alert was generated (HDR waited 10 seconds and no more flows arrived to new ports in that dst IP) then the counter resets and the same attacker needs to do _again_ more than threshold destinations ports in one IP to be detected. This avoids the problem that after 5 flows that generated an alert, the 6 flow also generates an alert.
...
HDR considers an IP performing a horizontal port scan if it contacted more than 6 destination IPs on the same specific port with not established connections. HDR checks both TCP and UDP connections for horizontal port scans. The initial threshold is now 6 destination IPs using the same destination ports.

After detecting a horizontal port scan, HDR waits 10 seconds to see if more flows arrive, since in most port scans the attcker will scan more ports. This avoids generating one port scan alert per flow in a long scan. Therfore HDR will wait until the scan finishes to alert on it. However, the first portscan is detected as soon as it happens so the analysts knows.

If one alert was generated (HDR waited 10 seconds and no more flows arrived to new IPs) then the counter resets and the same attacker needs to do _again_ more than threshold destinations IPs in the same port to be detected. This avoids the problem that after 6 flows that generated an alert, the 7 flow also generates an alert.

HDR ignores the broadcast IP 255.255.255.255 has destination of port scans.


### PING Sweeps

ICMP messages can be used to find out which hosts are alive in a network.
HDR relies on Zeek detections for this, but it is done with our own Zeek scripts located in
...
HDR does this detection using HDR' own zeek script located in
...
HDR detects when an IP is requesting 4, 8, 12, etc. different IPs from the DHCP server within the same
twimewindow (1 hour by default)

# Connections Made By HDR

HDR uses online databases to query information about many different things, for example (user agents, mac vendors etc.)

The list below contains all connections made by HDR
...
HDR is shipped with it's own custom zeek scripts to be able to extend zeek functionality and
