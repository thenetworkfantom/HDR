...
HDR can read the packets directly from the **network interface** of the host machine, and packets and flows from different types of files, including
...
All the input flows are converted to an internal format. So once read, HDR works the same with all of them.

After HDR was run on the traffic, the HDR output can be analyzed with Kalipso GUI interface. In this section, we will explain how to execute each type of file in HDR, and the output can be analyzed with Kalipso.

Either you are [running HDR in docker](https://stratospherelinuxips.readthedocs.io/en/develop/installation.html#installing-and-running-slips-inside-a-docker) or [locally](https://stratospherelinuxips.readthedocs.io/en/develop/installation.html#installing-slips-in-your-own-computer), you can run HDR using the same below commands and configurations.


## Reading the input

The table below shows the commands HDR uses for different inputs. The first part of the command **./slips.py -c config/slips.conf** is same, the second part changes depending on the input type. Also, the user can execute **./slips.py --help** to find correct argument to run HDR on each type of the file.
...
There is also a configuration file **config/slips.conf** where the user can set up parameters for HDR execution and models
...
HDR has 2 modes, interactive and daemonized.

**Daemonized** : means , output, logs and alerts are written in files.

In daemonized mode : HDR runs completely in the background, The output is written to``` stdout```, ```stderr``` and
...
If slips can't write there, slips will store the logs in the ```HDR/output/``` dir by default.
...
owned by root, you will need to run HDR using sudo or give the current user enough permission so that
...
By default, HDR will assume you are running only 1 instance and will use the redis port 6379 on each run.
...
If you want HDR to run on a certain port, you can use the ```-P <portnumber>``` parameter to specify the
port you want HDR to use. but it will always use port 6379 db 1 for the cache db.

Each instance of HDR will connect to redis server on a randomly generated port in the range (32768 to 32850).
...
Both redis servers, the main sever (DB 0) and the cache server (DB 1) are opened automatically by HDR.
...
NOTICE: if you run more than one instance of HDR on the same file or the same interface,
HDR will generate a new directory with the name of the file and the new timestamp inside the ```output/``` dir


## Closing redis servers

HDR uses a random unused port in the range in the range (32768 to 32850).

When running slips, it will warn you if you have more than 1 redis serve open using the following msg

    [Main] Warning: You have 2 redis servers running. Run HDR with --killall to stop them.
...
Due to issues running HDR on an interface on MacOS, we added the option ```-g```
...
## Reading the output
The output process collects output from the modules and handles the display of information on screen. Currently, HDR'
analysis and detected malicious behaviour can be analyzed as following:

- **Kalipso** - Node.JS based graphical user interface in the terminal. Kalipso displays HDR detection and analysis in colorful table and graphs, highlighting important detections. See section Kalipso for more explanation.
- **alerts.json and alerts.txt in the output folder** - collects all evidences and detections generated by HDR in a .txt and .json formats.
...
To open a new terminal inside HDR container first [run](https://stratospherelinuxips.readthedocs.io/en/develop/installation.html#running-slips-inside-a-docker-from-the-dockerhub) HDR in one terminal

Now in a new local terminal get the HDR container ID:

```docker ps```

Create another terminal of the HDR container using
...
2. You can use HDR' web interface by running slips with ```-w``` or running:
...
HDR uses redis to store analysis information. you can save your analysis for later use by running slips with ```-s```,
...
HDR allows you to whitelist some pieces of data in order to avoid its processing.
...
### Flows Whitelist
If you whitelist an IP address, HDR will check all flows and see if you are whitelisting to them or from them.

If you whitelist a domain, HDR will check:
- Domains in HTTP Host header
- Domains in the SNI field of TLS flows
- All the Domains in the DNS resolution of IPs (there can be many) (be careful that the resolution takes time, which means that some flows may not be whitelisted because HDR doesn't know they belong to a domain).
...
HDR uses Tranco whitelist which contains a research-oriented top sites
ranking hardened against manipulation here https://tranco-list.eu/

HDR download the top 10k domains from this list and by default and
whitelists all evidence and alerts from and to these domains. HDR still shows the flows to and from these IoC.


The tranco list is updated daily by default in HDR, but you can change how often to update it using the
...
HDR Support displaying popup notifications whenever there's an alert.
...
HDR doesn't need root permissions unless you
...
Even when HDR is run using sudo, it drops root privileges  in modules that don't need them.


## Modifying the configuration file

HDR has a ```config/slips.conf``` the contains user configurations for different modules and general execution. Below are some of HDR features that can be modifie with respect to the user preferences.

### Generic configuration

**Time window width.**

Each IP address that appears in the network traffic of the input is represented as a profile in HDR. Each profile is divided into time windows. Each time window is 1 hour long by default, and it gathers the network traffic and its behaviour for the period of 1 hour. The duration of the timewindow can be changed in the the config/slips.conf using

```time_window_width```

**Home Network**

HDR can use your home network if you want to focus on analysing
...
HDR needs to be run as root so it can execute iptables commands.
...
HDR can export alerts to different systems.
...
1. Running HDR with ```verbose``` and ```debug``` flags
...
HDR deletes all zeek log files after 1 day when running on an
...
By default, HDR rotates zeek files every 1 day. this can be changed in ```config/slips.conf```
...
HDR has an option to not delete the rotated zeek files immediately by setting the
...
####  Running HDR with verbose and debug flags
...
HDR also logs all errors to output/errors.log (interactive mode), and /var/log/slips/error.log (daemonized mode)
...
HDR supports reading input flows in text format from stdin in interactive mode.
...
HDR supports automatically running a custom zeek script by adding it to ```zeek-scripts``` dir and adding the file
...
Zeek output is suppressed by default, so if your script has errors, HDR will fail silently.

## HDR parameters

- ```-c``` or  ```--config``` Used for changing then path to the HDR config file. default is config/slips.conf
- ```-v``` or  ```--verbose``` Verbosity level. This logs more info about HDR.
...
- ```-p``` or  ```--blocking``` Allow HDR to block malicious IPs. Requires root access. Supported only on Linux.
...
- ```-w``` or  ```--webinterface``` Start HDR web interface automatically
- ```-V``` or  ```--version``` Used for checking your running HDR version flags.
- ```-im``` or  ```--input-module``` Used for reading flows from a module other than input process.


## Containing HDR resource consumption

When given a very a large pcap, slips may use more memory/CPU than it should. to fix that you can reduce the niceness of
HDR by running:

    renice -n 6 -p <HDR-PID>

## Running HDR from python

You can run HDR from python using the following script
