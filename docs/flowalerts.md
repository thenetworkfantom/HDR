...
If HDR runs by capturing packets directly from a network device (as opposed to, for example, a PCAP file), this detection will ignore all connections that happen in the first 3 minute of operation of HDR. This is because most times HDR is started when the computer is already running, and many DNS connections were already done. So waiting 3 minutes decreases the amount of False Positives.
...
HDR uses it's own lists of organizations and information about them (IPs, IP ranges, domains, and ASNs). They are stored in ```slips_files/organizations_info``` and they are used to check whether the IP/domain of each flow belong to a known org or not.

HDR doesn't detect 'connection without DNS' when running
...
HDR detects successful SSH connections using 2 ways
...
HDR has a list of known ports located in ```slips_files/ports_info/ports_used_by_specific_orgs.csv```
...
any port that isn't in the above 2 files is considered unknown to HDR.

HDR will detect established connections only to unknown ports.
...
HDR generates 'possible data upload' alerts when the number of uploaded bytes to any IP exceeds 100 MBs over
...
HDR also detects data upload when an IP uploads >=100MBs to any IP in 1 connections.

## Malicious JA3 and JA3s hashes

HDR uses JA3 hashes to detect C&C servers (JA3s) and infected clients (JA3)

HDR is shipped with it’s own zeek scripts that add JA3 and JA3s fingerprints to the SSL log files generated by zeek.

HDR supports JA3 feeds in addition to having more than 40 different threat intelligence feeds.
The JA3 feeds contain JA3 fingerprints that are identified as malicious.
The JA3 threat intelligence feed used by HDR now is Abuse.ch JA3 feed.
...
HDR detects any connection to port 0 using any protocol other than 'IGMP' and 'ICMP' as malicious.


## Multiple reconnection attempts

Multiple reconnection attempts in HDR are 5 or more not established flows (reconnections) to
the same destination IP on the same destination port.

## Zeek alerts

By default, HDR depends on Zeek for detecting different behaviours, for example
...
Some scans are also detected by HDR independently of Zeek, like ICMP sweeps and vertical/horizontal portscans.
Check
...
HDR alerts when 3+ invalid SMTP login attempts occurs within 10s
...
To detect DGA, HDR will count the amount of NXDOMAINs met in the DNS traffic of each source IP.
...
When HDR encounters a connection to or from a specific IP and a specific port, it scans previous connections looking for connection to/from that same IP using a different port.

It alerts when finding two or more connections to the same IP.

## Malicious SSL certificates

HDR uses SSL certificates sha1 hashes to detect C&C servers.

HDR supports SSL feeds and is shipped with Abuse.ch feed of malicious SSL hashes by default.
And you can add other SSL feeds in ```ssl_feeds``` in ```slips.conf```.


## Pastebin downloads

HDR detects downloads from pastebin using SSL and HTTP
...
HDR detects the pastebin download once the SSL connection is over , which may take hours.

## Young Domains

HDR uses whois python library to get the creation date of every domain met in the dns flows.
...
HDR uses zeek to detect SMTP connections,
When zeek detects a bad smtp login, it logs it to smtp.log, then slips reads
this file and sets an evidence.

## SMTP bruteforce

HDR detects a SMTP bruteforce when 3 or more bad SMTP
logins happen within 10 seconds.

---

With every generated evidence, HDR gathers as much info
...
HDR gathers AS, hostname, SNI, rDNS and any available data about this IP and you get an alert saying:
...
When slips enccounters a cn that claims to belong to any of HDR supported orgs (Google, Microsoft, Apple or Twitter)
HDR checks if the destination address or the destination server name belongs to these org.

If not, slips generates an alert.

## Weird HTTP methods

HDR uses zeek's weird.log where zeek logs weird HTTP methods seen in http.log
...
HDR detects established connections on port 443 that are not using HTTP
...
HDR detects established connections on port 80 that are not using SSL
using zeek's conn.log flows
...
HDR detects when a private IP is connected to another private IP with threat level info.

But it skips this alert when it's a DNS connection on port 53 UDP to the gateway

## Connection to private IPs outside the current local network

HDR detects the currently used local network and alerts if it find a
...
HDR check every DNS answer with TXT record for high entropy
strings.
...
HDR stores the MAC of each new IP it sees in conn.log.
...
HDR uses zeek tunnel.log to alert on GRE tunnels when found.
...
HDR detects this and sets an informational evidence.
